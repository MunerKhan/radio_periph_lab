...
void print_benchmark(volatile unsigned int *periph_base)
{
    ...
    const float clock_frequency = 125e6;
    unsigned int elapsed_cycles = stop_time - start_time;
    float time_spent = elapsed_cycles / clock_frequency;

    // Calculate bytes transferred and throughput
    unsigned int iterations = 2048;  
    unsigned int bytes_per_transfer = 4;
    unsigned int bytes_transferred = iterations * bytes_per_transfer;
    float throughput = (bytes_transferred / 1e6) / time_spent; // Convert to MB/s
...
// first, get a pointer to the peripheral base address using /dev/mem and the function mmap
    ...
    printf("\r\n\r\n\r\nLab 6 Muner Khan- Custom Peripheral Demonstration\n\r");
